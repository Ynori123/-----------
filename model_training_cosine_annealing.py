...
# 改进的MLP模型 - 添加批归一化和Dropout，并集成改进的余弦退火调度器
class ImprovedMLPWithCosineAnnealing(L.LightningModule):
    def __init__(self, input_size, hidden_size=128, output_size=2, dropout_rate=0.3, learning_rate=0.0005):
        super(ImprovedMLPWithCosineAnnealing, self).__init__()
    ...
    def configure_optimizers(self):
        ...
        warmup_scheduler = torch.optim.lr_scheduler.LinearLR(
            optimizer, 
            start_factor=0.1,  # 从初始学习率的10%开始
            end_factor=1.0,    # 逐渐增加到初始学习率
...
def train_model(df, input_columns, target_column, model_path, max_epochs=50, batch_size=256):
    """
    改进的模型训练函数，包含更好的数据处理和训练策略，使用余弦退火学习率调度
    """
    # 添加GPU诊断
    ...
    # 创建模型
    model = ImprovedMLPWithCosineAnnealing(
    ...
    trainer = L.Trainer(
        ...
        logger=L.pytorch.loggers.TensorBoardLogger('tb_logs/cosine_annealing', name='Z'),
    ...
    plt.title('Receiver Operating Characteristic (ROC) Model with Cosine Annealing LR')
    plt.legend(loc="lower right")
    plt.grid(True)
    plt.savefig('roc_curve_cosine_annealing.png', dpi=300, bbox_inches='tight')
    plt.show()
...
if __name__ == "__main__":
    # 请根据实际情况修改这些参数
    model_path = "./tb_logs/Z/version_cosine_annealing/best_model.ckpt"
